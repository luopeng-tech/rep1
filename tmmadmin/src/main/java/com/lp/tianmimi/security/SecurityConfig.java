...
import com.lp.tianmimi.security.handler.LoginErrorHandle;
import com.lp.tianmimi.security.handler.LoginSuccessHandle;
import com.lp.tianmimi.security.handler.LogoutSuccessHandle;
import com.lp.tianmimi.security.handler.PermissionErrorHandle;
import com.lp.tianmimi.service.AdminDetailService;
...
// 配置 Security 过滤请求的 例外
        web.ignoring().antMatchers("/admin/login", "/common/**", "/img/**", "/css/**", "/js/**", "/favic on.ico", "/assets/**")
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests() // 配置 请求需要进行权限认证
                ...
                .passwordParameter("password") // 账户密码的形参名称.passwordParameter("password") // 账户密码的形参名称
                ...
                .logoutSuccessHandler(new LogoutSuccessHandle()) // 安全退出成功处理 器
                .permitAll()
                .and()
                .csrf() // 跨域请求配置
                .disable() // 禁用
